...
class GCMCDataset(Dataset):
    def __init__(self, X, Y_log, LOW_log):
        ...
        self.LOW_log = torch.tensor(LOW_log, dtype=torch.float32).unsqueeze(1)

    def __len__(self): return len(self.X)
    def __getitem__(self, i): return self.X[i], self.Y_log[i], self.LOW_log[i]
...
def main(args):
    ...
    for it in range(max_iters):
        ...
        # Evaluate
        model.eval()
        X_rem = torch.tensor(scaler_X.transform(X_proc[idx_unlabeled]), dtype=torch.float32)
        with torch.no_grad():
            preds_log = model(X_rem).cpu().numpy()
        ...
        logging.info(f"[Eval] R2={r2:.4f}, MAE={mae:.6f}, MSE={mse:.6f}")
        ...
        # Query selection: Half QT + Half Uncertainty
        logging.info("[Query] Half Quantile + Half Uncertainty")
        half = args.samples_per_iter // 2

        # Quantile half
        strat_idx = stratified_quantile_sampling(LOW_log, idx_unlabeled, half, args.num_bins)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Half Quantile + Half MC Dropout AL for GCMC Surrogate")
