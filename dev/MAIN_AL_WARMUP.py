...
class GCMCDataset(Dataset):
    ...
    def __getitem__(self, i): 
        return self.X[i], self.Y[i], self.LOW[i]
...
def train_simple_epochs(model, optimizer, loss_fn, train_dl, scaler_X, epochs, relative):
    """단순 고정 Epoch 학습 (EarlyStopping 없음)"""
    for epoch in range(epochs):
        model.train()
        for xb, yb, lb in train_dl:
            feats = scaler_X.transform(xb)
...
def main(args):
    ...
    for it in range(max_iters):
        ...
        # (1) Warm-up 학습: 새 샘플만
        # -------------------------------
        logging.info(f"[Warmup] Training on {len(new_idx)} newly sampled data")
        warm_ds = GCMCDataset(X_all[new_idx], Y_all[new_idx], LOW_all[new_idx])
        warm_dl = DataLoader(warm_ds, batch_size=args.batch_size, shuffle=True)
        train_simple_epochs(model, optimizer, loss_fn, warm_dl, scaler_X,
                            epochs=args.warmup_epochs, relative=args.relative)

        # -------------------------------
        # (2) Full 학습: 전체 labeled 데이터
        # -------------------------------
        ...
        logging.info(f"[Full Training] Labeled={len(idx_labeled)} samples")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--warmup_epochs", type=int, default=20,
                        help="새로 들어온 샘플만 우선 학습할 Epoch 수")
