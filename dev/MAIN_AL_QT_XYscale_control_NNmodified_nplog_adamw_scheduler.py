...
def train_with_early_stopping(model, optimizer, scheduler, loss_fn, train_dl, val_dl,
                              epochs, patience):
    best_loss = float("inf")
    patience_ctr = 0
    best_state = None
...
def main(args):
    ...
    # --- Feature Transform: skewness 기반 log 변환 ---
    for j in range(X_all.shape[1]):
        sk = skew(X_all[:, j])
        if abs(sk) > args.skew_thresh or j == X_all.shape[1] - 1:  # 마지막(LOW)은 무조건 log
            X_all[:, j] = np.log(np.clip(X_all[:, j], 1e-12, None))

    # --- Target Transform ---
    Y_log = np.log(np.clip(Y_all, 1e-12, None))

    LOW_log = X_all[:, -1]  # 이미 log 적용됨
    ...
    if args.prefix == "auto":
        args.prefix = (
            ...
            f"lr{args.lr}_bs{args.batch_size}_wd{args.weight_decay}"
        )
    ...
    for it in range(max_iters):
        ...
        train_with_early_stopping(model, optimizer, scheduler, loss_fn,
                                  dl_train, dl_val, args.epochs, args.patience)
        ...
        # Query selection: RD + QT + Uncertainty
        ...
        uncert_order = np.argsort(uncertainties)[-UNC:] if UNC > 0 else []
        uncert_idx = idx_unlabeled[uncert_order] if UNC > 0 else np.array([], dtype=int)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AL with skew-based log, AdamW, ReduceLROnPlateau")
    ...
    parser.add_argument("--skew_thresh", type=float, default=2.0,
                        help="threshold for log-transform of skewed features")
