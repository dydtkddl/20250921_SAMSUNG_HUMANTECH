...
class GCMCDataset(Dataset):
    def __init__(self, X, Y_scaled, LOW_log):
        self.X = torch.tensor(X, dtype=torch.float32)
        self.Y = torch.tensor(Y_scaled, dtype=torch.float32).unsqueeze(1)
...
def main(args):
    ...
    if args.x_scale:
        ...
        scaler_X = Dummy()
    scaler_Y = StandardScaler().fit(Y_log[idx_labeled])

    # (4) 전체 데이터 transform
    X_scaled = scaler_X.transform(X_all)
    Y_scaled = scaler_Y.transform(Y_log).flatten()
    ...
    for it in range(max_iters):
        ...
        # Train
        d_train = GCMCDataset(X_scaled[idx_labeled], Y_scaled[idx_labeled], LOW_log[idx_labeled])
        ...
        unl_ds = GCMCDataset(X_scaled[idx_unlabeled], Y_scaled[idx_unlabeled], LOW_log[idx_unlabeled])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Active Learning with log+standard scaling (fit on initial pool)")
