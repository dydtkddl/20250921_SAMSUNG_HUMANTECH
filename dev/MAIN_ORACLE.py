...
def main(args):
    ...
    out_dir = f"{args.prefix}AL_Oracle_{ts}"
    ...
    for it in range(max_iters):
        ...
        # Oracle query selection
        logging.info("[Query] Oracle sampling based on true label errors...")
        X_unl = torch.tensor(scaler_X.transform(X_all[idx_unlabeled])).float()
        preds_mean_unl, _ = mc_dropout_predict(model, X_unl, args.mcd_n)
        y_pred_unl = preds_mean_unl.flatten() * LOW_all[idx_unlabeled] if args.relative else preds_mean_unl.flatten()
        y_true_unl = Y_all[idx_unlabeled]

        errors = np.abs(y_pred_unl - y_true_unl)
        oracle_order = np.argsort(errors)[-args.samples_per_iter:]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Oracle Sampling for Active Learning in GCMC Surrogate")
    ...
    parser.add_argument("--prefix", type=str, default="[Ar_05bar_5bar_Oracle]")
